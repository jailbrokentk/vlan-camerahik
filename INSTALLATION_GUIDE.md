...
3. **Khởi động và sử dụng:**
   ...
   * **Thêm thiết bị (NVR/Camera):** Vào mục **Cài đặt** (Settings) hoặc chọn **Thêm thiết bị** (Add Device), điền các thông tin:
     * Địa chỉ IP thiết bị (Ví dụ: `192.168.1.100`)
...
# Checkout sang nhánh làm việc chính
git checkout main
```
...
* `resources/sdk/`: Chứa các tệp DLL SDK Hikvision thiết yếu đã được tinh giản tối đa (như `HCNetSDK.dll`, `PlayM4.dll`, `SuperRender.dll` và thư mục con `HCNetSDKCom/...`) để tự động đính kèm vào phần mềm khi đóng gói.
