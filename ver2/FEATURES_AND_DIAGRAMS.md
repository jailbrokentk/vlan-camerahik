...
graph TD
    ...
    subgraph Native [C++ Addon & Win32]
        Addon[hcnet-addon.node]
        Win32[Win32 API: CreateWindowEx]
    end

    subgraph SDK [Hikvision SDK]
        HCNet[HCNetSDK.dll]
...
sequenceDiagram
    ...
    Cell->>DO: Show Global Drag Overlay (orange borders)
...
sequenceDiagram
    ...
    User->>UI: Drag Timeline or Click "Đi đến"
    UI->>UI: triggerSeekLock(1500ms) (Set isSeekingRef = true)
    Note over UI,Polling: Polling is temporarily FROZEN / IGNORED
    ...
    Note over UI: 1.5s timeout expires (Set isSeekingRef = false)
    Polling->>Addon: Resume polling GetPlaybackTime()
    Addon->>SDK: NET_DVR_PLAYGETTIME
    SDK-->>Polling: Return correct timestamp (newTime + progress)
...
graph TD
    ...
    subgraph Expanded_Video [Scaled C++ Window]
        Video_Rect[Scale: 1.0x to 4.0x + Pan offset]
    end
