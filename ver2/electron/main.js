...
ipcMain.handle('hcnet:startPlay', async (event, deviceId, channelId, streamType, x, y, w, h, enableSmart) => {
  const devices = store.get('devices') || []
  ...
  // Ưu tiên giao thức kết nối được cấu hình riêng cho thiết bị, fallback về cài đặt chung toàn cục
  const linkMode = Number(dev.linkMode ?? store.get('liveLinkMode') ?? 0)
  const bufNum = Number(store.get('liveBufferFrames') ?? 3)
