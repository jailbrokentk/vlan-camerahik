...
Napi::Value StartRealPlay(const Napi::CallbackInfo& info) {
    ...
    // Hikvision SDK cấm sử dụng Real-time mode (byPreviewMode = 1) khi kết nối bằng TCP (dwLinkMode = 0).
    // Nếu kết hợp TCP + Real-time, SDK sẽ ném ra lỗi NET_DVR_NOOPENTRANSMOD (Code 23).
    // Vì vậy, ta bắt buộc phải đặt byPreviewMode = 0 (Normal preview) khi dùng TCP.
    ...
    // Set buffer number to limit maximum cached frames and eliminate cumulative delay
    if (bufNum > 0) {
        BOOL setBufOk = NET_DVR_SetPlayerBufNumber(previewHandle, bufNum);
        printf("[addon] SetPlayerBufNumber handle=%ld bufNum=%d result=%d\n", previewHandle, bufNum, setBufOk);
    }
