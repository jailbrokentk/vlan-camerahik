...
export const useStore = create((set, get) => ({
  ...
  // Bitrate Manager — giới hạn bitrate tối đa (KB/s), 0 = không giới hạn
  sdBitrateLimit: 32,   // SD Sub Stream mặc định: 32 KB/s (256 Kbps)
  hdBitrateLimit: 256,  // HD Main Stream mặc định: 256 KB/s (2048 Kbps)
  setSdBitrateLimit: (sdBitrateLimit) => set(() => {
    const api = getAPI()
    if (api) {
      api.storeSet('sdBitrateLimit', sdBitrateLimit).catch((e) => console.error('Failed to save SD bitrate limit:', e))
    }
    return { sdBitrateLimit }
  }),
  setHdBitrateLimit: (hdBitrateLimit) => set(() => {
    const api = getAPI()
    if (api) {
      api.storeSet('hdBitrateLimit', hdBitrateLimit).catch((e) => console.error('Failed to save HD bitrate limit:', e))
    }
    return { hdBitrateLimit }
  }),

  // State lưu trữ bitrate & decodeMode thực tế của từng luồng
  streamStats: {},
  pollStreamStats: async (fullscreenUiKey, focusedUiKey) => {
    const api = getAPI()
    if (api) {
      try {
        const stats = await api.hcnetGetStreamStats(fullscreenUiKey, focusedUiKey)
        if (stats) {
          set({ streamStats: stats })
        }
      } catch (e) {
        console.error('Failed to poll stream stats:', e.message)
      }
    }
  },
