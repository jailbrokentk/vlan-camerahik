...
export function PlaybackView({ camera, onClose, onChangeCamera }) {
  ...
  async function triggerDownload() {
    ...
    try {
      ...
      if (api?.downloadFile) {
        // Lắng nghe tiến độ tải thực tế và trạng thái convert từ backend
        const removeListener = window.electronAPI.onDownloadProgress((event, data) => {
          if (data.status === 'downloading') {
            setDownloadProgress(Math.min(95, Math.round(data.percent)));
            ...
            {downloading && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11 }}>
                  <span>{isConverting ? t('playback.converting') : t('playback.downloading')}</span>
