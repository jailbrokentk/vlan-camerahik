...
export default function LiveView({ panelId }) {
  ...
  const streamResolution = useStore(s => s.streamResolution)
  const { t } = useLanguage()
  const targetPanelId = panelId || activePanelId
  const panel = panels.find(p => p.id === targetPanelId)
  const api = getAPI()
  const pollStreamStats = useStore(s => s.pollStreamStats)
  ...
  // Định kỳ poll dữ liệu tốc độ bitrate & trạng thái giải mã từ main process mỗi 1 giây
  const focusedCellIndex = useStore(s => s.focusedCellIndex)
  useEffect(() => {
    const timer = setInterval(() => {
      let fullscreenUiKey = null
      if (fsState.cameraKey && (fsState.phase === 'entering' || fsState.phase === 'camera')) {
        const parts = fsState.cameraKey.split(':')
        if (parts.length >= 4) {
          fullscreenUiKey = `${parts[2]}_${parts[3]}` // Tạo khóa dạng deviceId_channelId
        }
      }

      let focusedUiKey = null
      const focusedSlot = panel?.cameraSlots[focusedCellIndex]
      if (focusedSlot) {
        focusedUiKey = `${focusedSlot.deviceId}_${focusedSlot.channelId}`
      }

      pollStreamStats(fullscreenUiKey, focusedUiKey)
    }, 1000)
    return () => clearInterval(timer)
  }, [pollStreamStats, fsState.cameraKey, fsState.phase, focusedCellIndex, panel])
  ...
  const handleGridContextMenu = async (e) => {
    ...
    // Xác định chính xác chất lượng luồng thực tế của ô camera từ Bitrate Manager hoặc lựa chọn cụ thể
    const defaultQuality = panel?.streamResolution || streamResolution || 'HD'
    const explicit = explicitQualities[cameraKey]
    const resolvedQuality = explicit || defaultQuality
      ...
      <div style={{ display: 'flex', flex: 1, flexDirection: 'row', overflow: 'hidden' }}>
        ...
        >
          {Array.from({ length: cellCount }).map((_, i) => {
            ...
            const desiredQuality = explicit || defaultQuality
            let cellStyle = {}
            if (isAnyFullscreen) {
              if (isSelected) {
                cellStyle = {
