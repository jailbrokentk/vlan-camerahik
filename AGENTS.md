...
## i18n & Multi-language (Đa Ngôn Ngữ)

- **CẤM TUYỆT ĐỐI** viết hardcode chuỗi text hiển thị lên giao diện (tiếng Việt hay tiếng Anh) vào trực tiếp mã nguồn Frontend (React) hay Backend (Electron).
- Mọi chuỗi text hiển thị phải được định nghĩa trong hệ thống file ngôn ngữ:
  - Tiếng Anh (mặc định): [en.js](file:///f:/ivms/src/i18n/en.js)
  - Tiếng Việt: [vi.js](file:///f:/ivms/src/i18n/vi.js)
- Ở Frontend, bắt buộc sử dụng hook `const { t } = useLanguage()` và gọi `t('lang.key')` để hiển thị text.
- Vi phạm rule này sẽ dẫn đến việc code bị từ chối/bị phạt nặng.

## Version Isolation (Cô Lập Phiên Bản v1 & v2)

- **CẤM TUYỆT ĐỐI** viết code lẫn lộn hoặc chỉnh sửa đè chéo giữa Version 1 và Version 2.
- **Khi làm việc với Version 1:** Chỉ thao tác trên mã nguồn tại thư mục gốc (root: `/src`, `/electron`, `/hcnet-addon`, v.v.).
- **Khi làm việc với Version 2:** Chỉ thao tác trên mã nguồn nằm hoàn toàn trong thư mục con `/ver2` (ví dụ `/ver2/src`, `/ver2/electron`, `/ver2/hcnet-addon`, v.v.).
- Đảm bảo tính độc lập và toàn vẹn của cả hai phiên bản trên nhánh `main`.
