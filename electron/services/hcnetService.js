...
// Helper tự đọc file cấu hình config.json của ứng dụng mà không phụ thuộc thư viện ngoài
function readStoreConfig() {
  try {
    ...
    console.error('[HCNetService] readStoreConfig failed:', err.message)
  }
  return {}
}
...
class HCNetService {
  ...
  init() {
    ...
    try {
      ...
      if (ok) {
        ...
        this.startBitrateController() // Khởi động vòng lặp kiểm tra bitrate
      } else {
  ...
  cleanup() {
    ...
    this.stopBitrateController() // Dừng vòng lặp kiểm tra bitrate
  ...
  _resolveSDKChannel(session, channelId, deviceChannels, deviceId) {
  ...
  startPlay(deviceId, channelId, streamType, parentHwnd, x, y, w, h, ip, port, username, password, deviceChannels, enableSmart, linkMode = 0, bufNum = 3, bitrateLimitKBps = 0, initialDecodeFrameType = 0) {
    const session = this.login(deviceId, ip, port, username, password)
    const sdkChannel = this._resolveSDKChannel(session, channelId, deviceChannels, deviceId)
    ...
    try {
      const previewMode = -1
      console.log(`[HCNetService] Starting real play. UI ch: ${channelId} → sdkChannel: ${sdkChannel}, dwStreamType: ${streamType}, enableSmart: ${enableSmart}, linkMode: ${linkMode}, bufNum: ${bufNum}, bitrateLimit: ${bitrateLimitKBps} KB/s, initialDecode: ${initialDecodeFrameType}`)
      const result = addon.startRealPlay(parentHwnd, session.userId, sdkChannel, streamType, x, y, w, h, !!enableSmart, linkMode, bufNum, previewMode, initialDecodeFrameType)
      ...
      this.activeStreams.set(streamKey, {
        ...
        channelId,
        streamType,
        decodeMode: initialDecodeFrameType, // 0 = Full frame, 1 = I-Frame only
        lastBitrate: 0,
        bitrateLimitKbps: (bitrateLimitKBps || 0) * 8 // Quy đổi KB/s sang Kbps để so sánh
      })
      ...
      // Xóa session lỗi để buộc đăng nhập lại từ đầu ghi ở lần thử tiếp theo (Refresh)
      this.sessions.delete(deviceId)
      throw err
    }
  }
  ...
  startPlayback(deviceId, channelId, startTimeStr, endTimeStr, parentHwnd, x, y, w, h, ip, port, username, password, deviceChannels) {
    ...
    try {
      ...
      // Xóa session lỗi khi playback gặp lỗi để buộc login lại
      this.sessions.delete(deviceId)
      throw err
    }
  }
  ...
  // ─── Client-side Bitrate & Frame-rate Controller (Tầng Service) ──────────────

  startBitrateController(intervalMs = 2000) {
    if (this.bitrateTimer) return
    this.bitrateTimer = setInterval(() => {
      this.checkAndThrottleStreams()
    }, intervalMs)
    console.log(`[HCNetService] Client-side Bitrate Controller loop started (Interval: ${intervalMs}ms)`)
  }
  ...
  stopBitrateController() {
    if (this.bitrateTimer) {
      ...
      this.bitrateTimer = null
      console.log('[HCNetService] Client-side Bitrate Controller loop stopped')
    }
  }

  checkAndThrottleStreams() {
    if (!this.isAvailable()) return

    // Đọc động cấu hình panels và các giới hạn bitrate từ file config.json của ứng dụng để cập nhật ngay lập tức
    const data = readStoreConfig()
    const panels = data.panels || []
    const activePanelId = data.activePanelId || 'panel-default'
    const panel = panels.find(p => p.id === activePanelId)
    const globalSdLimit = data.sdBitrateLimit ?? 32
    const globalHdLimit = data.hdBitrateLimit ?? 256

    for (const [streamKey, stream] of this.activeStreams) {
      try {
        const previewHandle = stream.previewHandle
        if (previewHandle >= 0) {
          const currentKbps = addon.getStreamBitrate(previewHandle)

          // Xác định giới hạn động (KB/s) theo loại luồng streamType (0=HD, 1=SD)
          const isSd = stream.streamType === 1
          const panelLimit = isSd ? panel?.sdBitrateLimit : panel?.hdBitrateLimit
          const globalLimit = isSd ? globalSdLimit : globalHdLimit
          const currentLimitKBps = panelLimit !== undefined ? panelLimit : globalLimit
          const limitKbps = (currentLimitKBps || 0) * 8

          // Lưu bitrate thực tế để hiển thị (không giới hạn khung hình, chỉ monitor)
          stream.lastBitrate = currentKbps

          // Đánh dấu trạng thái: vượt ngưỡng hay không (chỉ để hiển thị cảnh báo, không bóp giải mã)
          const isOverLimit = limitKbps > 0 && currentKbps > limitKbps
          stream.decodeMode = isOverLimit ? 1 : 0 // 1 = vượt ngưỡng (cảnh báo), 0 = bình thường
        }
      } catch (err) {
        console.error(`[HCNetService] Bitrate check failed for ${streamKey}:`, err.message)
      }
    }
  }

  getStreamStats(fullscreenUiKey, focusedUiKey) {
    this.activeFullscreenUiKey = fullscreenUiKey // Ghi nhận khóa camera đang phóng to
    this.activeFocusedUiKey = focusedUiKey // Ghi nhận khóa camera đang được focus
    
    const stats = {}
    for (const [streamKey, stream] of this.activeStreams) {
      stats[`${stream.deviceId}_${stream.channelId}`] = {
        bitrateKbps: stream.lastBitrate || 0,
        decodeMode: stream.decodeMode === 1 ? 'Over-limit' : 'Normal',
        limitKbps: stream.bitrateLimitKbps || 0
      }
    }
    return stats
  }
